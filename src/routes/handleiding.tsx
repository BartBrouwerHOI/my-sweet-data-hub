...
cd /opt/supabase && sudo docker compose restart auth`}</CodeBlock>
      ...
      <Step id="troubleshooting" number={steps.findIndex(s => s.id === "troubleshooting") + 1} title="Troubleshooting">
        <div className="space-y-4">
          ...
          <TroubleItem q="Migratie mislukt met foreign key fout (verse DB)" a="Dit komt vaak door hardcoded user-IDs of seed-data uit Lovable Cloud die niet bestaan op een schone database. Twee opties: (1) Fix de migratie in je app-repo — maak INSERTs conditioneel met WHERE EXISTS (...) of gebruik een trigger op auth.users voor de eerste admin. (2) Skip de migratie eenmalig op de server: sudo lovable-update --mark-done <bestandsnaam.sql> en daarna sudo lovable-update. Volledige richtlijnen voor app-developers staan in docs/APP-CONTRACT.md in de infra-repo." />
          <TroubleItem q="'⚠️ INSTALLATIE DEELS VOLTOOID'" a="De infrastructuur draait prima, maar een app-migratie is mislukt. Dit is geen infra-probleem — de fix moet in je app-repo. Zie de foutmelding voor details, en de richtlijnen in docs/APP-CONTRACT.md (in de infra-repo) voor hoe je migraties idempotent maakt. Na de fix: push naar GitHub en draai lovable-update." />
